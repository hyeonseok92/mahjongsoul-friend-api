...
const { URL_BASE, ACCESS_TOKEN, UID, PREFERRED_SERVER, OAUTH_TYPE} = require("./majsoul_env");
...
async function createMajsoulConnection(accessToken = ACCESS_TOKEN, uid = UID, preferredServer = PREFERRED_SERVER) {
  ...
  const type = parseInt(OAUTH_TYPE) || 5;
...
//      console.log(conn.clientVersionString);
//      console.log("type: " + type);
      const oauth2Auth = await conn.rpcCall(".lq.Lobby.oauth2Auth", {
        type: type,
        code: accessToken,
        uid: uid,
        client_version_string: conn.clientVersionString,
      });
//      console.log(oauth2Auth.access_token);
      let oauth2Check = await conn.rpcCall(".lq.Lobby.oauth2Check", {
        type: type,
        access_token: oauth2Auth.access_token
      });
//      console.log(oauth2Check);
      if (!oauth2Check.has_account) {
        await new Promise((res) => setTimeout(res, 2000));
        oauth2Check = await conn.rpcCall(".lq.Lobby.oauth2Check", { type: type, access_token: oauth2Auth.access_token });
      }
      assert(oauth2Check.has_account);
      let oauth2Login = await conn.rpcCall(".lq.Lobby.oauth2Login", {
        type: type,
        access_token: oauth2Auth.access_token, //accessToken
        reconnect: false,
        device: {
          platform: "pc",
          hardware: "pc",
